...
def PolicyIteration(n):
    ...
    while True:
        ...
        while True:
            ...
            for i in range(numValStates):
                ...
                if state[a] == '0' and state.count('1') == state.count('2') and (isGameOver(state,n) == 0):
                    ...
                    for ns in next_states:
                        ...
                        v += rwd + gamma*old_vpi[indexOfState[ns]]
            ...
            max_diff = 0
            for i in range(numValStates):
                max_diff = max(max_diff,abs(v_pi[i][0] - old_vpi[i][0]))
            ...
            for a in range(n*n):
                ...
                if state[a] == '0' and state.count('1') == state.count('2') and (isGameOver(state,n) == 0):
                    ...
                    for ns in next_states:
                        ...
                        v += rwd + gamma*v_pi[indexOfState[ns]]
                    v /= len(next_states)
                # maxa = max(maxa, w_i0)
                if v > maxa:
                    maxa = v
        ...
        policy_diff = 0
        for i in range(numValStates):
            policy_diff = max(policy_diff, abs(old_policy[validStates[i]] - policy[validStates[i]]))
...
def randomPolicy(state):
    ...
    for i in range(len(state)):
        if state[i] == '0':
            remPos.append(i)
    index = random.randint(0,len(remPos)-1)
    return remPos[index]
...
def QLearning(n):
    ...
    QTable = {s:[0 for i in range(n*n)] for s in validStates}
    ...
    for e in range(episodes):
        ...
        while True:
            # print(1)
            a = randomPolicy(s)
            next_states = transition_func(s, a, n)
            
            next_s = next_states[random.randint(0,len(next_states)-1)]
            win = isGameOver(next_s, n)
            rwd = 0
            if win == 1:
                rwd = 2
            if win == 2:
            ...
            QTable[s][a] = QTable[s][a] + lr*(rwd + gamma*max(QTable[next_s]) - QTable[s][a])
    ...
    for st in validStates:
        policy[st] = np.argmax(np.array(QTable[st]))
